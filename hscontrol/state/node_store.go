...
func snapshotFromNodes(
	...
	if reuseRoutes {
		...
		// primary survive unrelated batches. Routes are keyed per user
		// scope, so the previous per-scope election is carried over.
		var prevRoutes map[types.UserID]map[netip.Prefix]types.NodeID
		if prev != nil {
